...
from src.entrypoints import (
    control_redis_eventconsumer_missing_entity_fetched,
    control_redis_eventconsumer_update_data_prepared,
    control_redis_eventpublisher,
)
...
def subscribe_updated_entity_prepared():
    ...
    while time.time() - start_time <= duration:
        message = pubsub.get_message(timeout=1)
        if message:
            control_redis_eventconsumer_update_data_prepared.handle_message(
