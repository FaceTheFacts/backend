...
def append_votes_json():
    missing_votes = load_json_data("votes", models.Vote)
    if missing_votes:
        poll_ids = load_entity_ids_from_db(models.Poll)
        votes = []
        for missing_vote in missing_votes:
...
def append_politicians_json():
    missing_politicians = load_json_data("politicians", models.Politician)
    if missing_politicians:
        politicians = [
            {
                "id": api_politician["id"],
